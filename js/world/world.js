class World {
  constructor(width, height, elevation, smoothing) {
    let heightMap = this.generateHeightMap(width, height, elevation, smoothing)
    this.map = []
    this.chars = []
    for(let y in heightMap) {
      this.map.push([])
      this.chars.push([])
      for(let x in heightMap) {
        this.map[y].push(new Place(heightMap[y][x]));
        this.chars[y].push(this.map[y][x].getChar());
      }
    }
  }

  generateHeightMap(width, height, elevation, smoothing) {
